...
DATASET_PATH = ROOT / "data" / "relation_pairs_fixed.json"
OUTPUT_DIR = ROOT.parent / "models" / "relation_classifier"
MAX_LENGTH = 256
BATCH_SIZE = 16
LEARNING_RATE = 3e-5
NUM_EPOCHS = 5
...
def build_pairs(raw: List[Dict]) -> List[Tuple[str, str, str]]:
    ...
    for item in raw:
        if {"claim_text", "evidence_text", "label"}.issubset(item):
            pairs.append((item["claim_text"], item["evidence_text"], item["label"]))
            continue
...
def stratified_split(
    ...
    """Split relation pairs while preserving support/attack/neutral balance."""
    by_label: Dict[str, List[Tuple[str, str, str]]] = {label: [] for label in LABEL2ID}
    for pair in pairs:
    ...
    for label_pairs in by_label.values():
        ...
        split_idx = max(1, int(len(label_pairs) * validation_ratio))
        val_pairs.extend(label_pairs[:split_idx])
        train_pairs.extend(label_pairs[split_idx:])
